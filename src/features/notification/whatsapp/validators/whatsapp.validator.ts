...
export class WhatsAppValidator {
  ...
  /**
   * Prevents duplicate sends by checking if an identical message template
   * was sent to the same mobile number within a short window (e.g. 10 seconds).
   */
  async isDuplicateSend(toPhone: string, templateName: string, windowSeconds: number = 10): Promise<boolean> {
    ...
    if (error) {
      console.error('[WhatsAppValidator] Error checking duplicates:', error)
      return false
    }
  ...
  /**
   * Masks sensitive fields (e.g., OTP codes, passwords) in dynamic variables.
   */
